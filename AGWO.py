...
class GreyWPlanner:
    ...
    def plan_path(self) -> Optional[np.ndarray]:
        """Plan path using improved GWO, running all iterations"""
        best_path = None
        best_length = float('inf')
        total_iterations = 300  # Fixed number of iterations
            ...
            if len(current_path) > 1:
                ...
                if np.linalg.norm(current_path[-1] - self.goal) < self.resolution * 2:
                    ...
                    if path_length < best_length:
                        ...
                        # Remove early termination condition to ensure all iterations run
